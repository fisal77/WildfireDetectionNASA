...
from efficientnet.datasets.fire import DataLoader
...
## [TODO] make a test.py which runs efficientnet in real-time on opencv
...
def evaluate(model, valid_loader, device):
    ...
    with torch.no_grad():
        ...
        ## [TODO] load data from fire.py's dataloader
        #valid_loader = tqdm(valid_loader, desc='Validate', ncols=0)
        valid_loader = DataLoader()
        for img, lb in valid_loader:
            img = img.to(device)
            lb = lb.to(device)
    ...
    return loss#valid_loss, valid_acc
...
if __name__ == '__main__':
    ...
    valid_loader = DataLoader(args.root, image_size, False, args.batch_size, num_workers=args.num_workers)
